...
ПРИМЕР ОТВЕТА:
{
  ...
  "action": "Сегодня спроси одного коллегу, заметил ли он опоздание — одна короткая проверка мысли."
}
```

**v1.2 Response depth**: поле `action` — один наблюдаемый шаг на сегодня (мини-эксперимент Бёрнса, ≤ 30 слов). UI: action → reframing → question → distortions.
...
Ответ:
```json
{"distortions": [...], "reframing": "...", "question": "...", "action": "...", "pattern": "..."}
```

`action` — часть контракта v1.2; клиент терпит отсутствие поля (старые/частичные ответы).
