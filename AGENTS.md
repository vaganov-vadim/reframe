# AGENTS.md — Reframe

> Инструкция для AI-агентов, работающих с проектом. Прочитай перед любыми изменениями.

## О проекте

**Reframe** — приватный голосовой КПТ-дневник для профессионалов под стрессом. Пользователь говорит → LLM анализирует через призму когнитивно-поведенческой терапии (Бёрнс) → рефрейминг.

Все данные — только на устройстве пользователя. Бэкенд — тонкий прокси к LLM без хранения.

## Методология: Spec-Driven Development (Speckit)

Мы НЕ пишем код до спецификации. Работаем по циклу:

```
constitution → specify → plan → tasks → implement
```

### Артефакты (всегда актуальны)

| Файл | Что определяет | Когда читать |
|---|---|---|
| `.specify/memory/constitution.md` | Принципы, архитектура, границы, стек | Перед любым решением |
| `.specify/memory/spec.md` | Функциональные требования, сценарии | Перед plan и implement |
| `.specify/memory/plan.md` | Технический план, структура, CI, pre-commit | Перед implement |
| `.specify/memory/tasks.md` | Список задач, фазы, статус выполнения | Перед implement |
| `AGENTS.md` | Этот файл | При входе в проект |

### Порядок работы

1. **Перед любым изменением** — прочитай constitution.md. Если предлагаемое решение противоречит конституции — остановись, предложи изменить конституцию.
2. **Новая фича** — `/speckit.specify` → `/speckit.plan` → `/speckit.tasks` → `/speckit.implement`
3. **Баг или правка** — проверь, не противоречит ли исправление конституции. Если нет — правь, с тестами.
4. **⚠️ ВСЕГДА сначала обновляй speckit-артефакты, потом код.** Никакой реализации до обновления spec/plan/tasks.
5. **⚠️ Новая фича → acceptance-тест ДО реализации.** Без E2E/acceptance-теста PR не мёрджится.
6. **Каждая новая задача — от свежего main.** `git checkout main && git pull` перед `git checkout -b feature/xxx` или `fix/xxx` (см. Git → Commit style).

> **Примечание по `/speckit.implement`**: вместо встроенного инструмента speckit мы используем `/start-work` + OpenCode-агентов для реализации. Это даёт полный контроль над контекстом (constitution, spec, plan, design tokens) и позволяет параллельное выполнение независимых задач. Результат тот же — tasks.md → код → CI/CD → продакшен.

### ⚠️ ВСЕГДА сначала обновляй speckit-артефакты, потом код.

Никакой реализации до обновления spec/plan/tasks. Если пользователь просит фичу:

1. Обнови spec.md (что делаем)
2. Обнови plan.md (как делаем)
3. Обнови tasks.md (какие задачи)
4. Только потом — код

Исключение: баг-фиксы (меньше 5 строк), изменения в документации.

### Правила для AI-оркестратора

- **Дроби задачи.** Spec → отдельно. Plan → отдельно. Код → отдельно. Один агент = одна задача.
- **Упало → скажи сразу.** Без молчания. «Сорвалось, перезапускаю.»
- **Фон для длинных задач.** `run_in_background=true`, потом проверять статус.
- **Мёрдж только после зелёного CI.** Без исключений.

## Технический стек

| Слой | Технология |
|---|---|
| Фронтенд | TypeScript (strict), React, Vite |
| Хранение | localStorage |
| Unit-тесты | Vitest |
| E2E-тесты | Playwright |
| Бэкенд | Clojure + Kit (прокси + промпт-инжиниринг) |
| API | REST + JSON |

## Ключевые правила

### Код
- **TypeScript strict mode** — никаких `any`

### Clojure: REPL-Driven Development (ИДИОМАТИЧНО)

Clojure — язык с REPL (Read-Eval-Print Loop). Это не просто консоль, а **основной способ разработки**. Вместо цикла «написал → скомпилировал → запустил → проверил» используется мгновенная обратная связь: оценил выражение → увидел результат → повторил.

**Почему REPL — это главное преимущество Clojure** (источник: [clojure.org](https://clojure.org/guides/repl/introduction)):

> REPL даёт программисту интерактивный опыт разработки. При создании новой функциональности он позволяет строить программы, сначала выполняя маленькие задачи вручную, как если бы программист *был* компьютером, затем постепенно автоматизируя их, пока желаемая функциональность не будет полностью запрограммирована.

**Что делать через REPL (вместо других инструментов):**

| Задача | Вместо | Через REPL |
|---|---|---|
| Запуск dev-окружения | Docker, скрипты | `lein repl` |
| Прогон тестов | CI, отдельный терминал | `(clojure.test/run-tests)` прямо в REPL |
| Отладка | println, логи | Оценить проблемную функцию с тестовыми данными, увидеть результат |
| Изучение API библиотек | Чтение доков | `(doc some-fn)`, `(source some-fn)`, вызвать с разными аргументами |
| Прототипирование промптов | Править prompt.clj → перезапускать сервер | Вызвать `build-prompt` с тестовым текстом, сразу увидеть результат |
| Исследование багов | Читать код, гадать | Воспроизвести проблему в REPL, сузить причину |

**Как использовать REPL в проекте Reframe:**

```bash
cd backend
lein repl
```

```clojure
;; Загрузить неймспейсы проекта
(require '[reframe.prompt :as p])
(require '[reframe.llm-client :as llm])
(require '[reframe.rate-limiter :as rl])

;; Прототипирование промпта
(println (p/build-prompt "Я опоздал на встречу"))

;; Тестирование функции изолированно
(p/build-deeper-prompt "Я опоздал на встречу" "Я безответственный")

;; Прогон тестов без выхода из REPL
(clojure.test/run-tests 'reframe.handler-test)

;; Перезагрузка изменённого неймспейса
(require '[reframe.prompt :as p] :reload)
```

**REPL + TDD (совместимы):**
1. Написать тест (RED)
2. В REPL: вызвать тестируемую функцию, проверить логику вручную
3. Дописать код, перезагрузить неймспейс (`:reload`)
4. Прогнать тест в REPL — зелёный
5. `lein test` — финальная проверка

**Принцип:** используй REPL для исследования и быстрой обратной связи. Не угадывай — проверь в REPL за секунду.

### TDD (НЕНАРУШИМО)

1. **Сначала тест** — пишем тест, который падает (RED)
2. **Потом код** — минимальные изменения чтобы тест прошёл (GREEN)
3. **Финально рефакторинг** — улучшаем код, тест всё ещё зелёный

Для фронтенда: Vitest (unit) или Playwright (E2E)
Для бэкенда: clojure.test

**Без теста PR не принимается.** Исключение: изменения в документации (.md).
- **Pre-commit gates** — перед коммитом: `tsc --noEmit` (чисто), `eslint` (чисто), `vitest run` (зелено). Корневой `lefthook.yml` — авторитативный (`root: frontend`). `frontend/lefthook.yml` — дубликат без `root:`.
- **Никакого мёртвого кода**, закомментированных блоков, TODO без issue

### Pre-push checks (ОБЯЗАТЕЛЬНО перед пушем)

Перед `git push` локально прогнать ВСЕ проверки:

```bash
# Фронтенд
cd frontend
npx tsc --noEmit          # типы чисты
npx eslint .              # линтер чист
npx vitest run            # unit-тесты зелёные
npx playwright test       # E2E-тесты зелёные

# Бэкенд
cd ../backend
lein test                 # backend-тесты зелёные
```

**Никаких пушей с красными тестами.** Если CI упал — чини локально, перепрогоняй все проверки, только потом пуш.

### PR-only workflow (НЕНАРУШИМО)

**Любые изменения — только через Pull Request.** Без исключений.

1. Ветка → коммит → push → PR → CI зелёный → мёрдж
2. **Запрещено**: прямой пуш в main, мёрдж без зелёного CI, мёрдж не глядя на CI
3. **Trivial changes** (1 файл, опечатка, удаление) — тоже через PR. Никаких «это же мелочь».
4. Если CI упал — чини в ветке, пуши, жди зелёного
5. Мёрдж только после APPROVE всех required checks (lint, test, build, e2e)

### Архитектура
- **Бэкенд НЕ хранит данные** — ни тексты, ни логи с содержимым
- **Фронтенд НЕ содержит API-ключей** и не обращается к LLM напрямую
- **LLM-провайдер заменяем** без изменения фронтенда (контракт на бэкенде)
- **Промпт-инжиниринг** (методология Бёрнса) — на бэкенде
- **Бизнес-логика клиента**: валидация, сессии, история, UX

### Приватность
- Все пользовательские данные ТОЛЬКО в localStorage
- История сессий НЕ содержит текст записей или рефрейминга — только метаданные
- Никакой телеметрии с содержимым

### Дизайн
- Тёплая нейтральная палитра, тёмная тема по умолчанию
- Минимальная зона касания 48px
- Ориентиры: Calm, Headspace, Nivra
- Максимум 2–3 действия на экране

## Структура проекта

```
reframe/
├── frontend/          # TypeScript + React SPA
│   ├── src/
│   │   ├── components/
│   │   ├── hooks/
│   │   ├── services/  # Бизнес-логика клиента
│   │   ├── store/
│   │   └── types/
│   └── tests/
│       ├── unit/      # Vitest
│       └── e2e/       # Playwright
├── backend/           # Clojure + Kit прокси
│   ├── src/
│   └── test/
├── .specify/          # Speckit артефакты
│   └── memory/
│       ├── constitution.md
│       ├── spec.md
│       └── plan.md
└── .omo/              # Планы, отложенные требования и transient-артефакты
    ├── constitution-updates.md
```

## Что НЕ надо предлагать (уже решено)

- ❌ Заменить Clojure на Node.js — осознанный выбор
- ❌ Добавить серверную БД — противоречит приватности
- ❌ Текстовый ввод вместо голоса (v1) — осознанно voice-only, будет в v2
- ❌ Server-side rendering, сложный роутинг — противоречит простоте
- ❌ Мобильное приложение (native) — веб-приложение достаточно

## Git

### Commit style
Стандартный GitHub-стиль: [Conventional Commits](https://www.conventionalcommits.org/).

```
type(scope): краткое описание в повелительном наклонении
```

**Типы**: `docs`, `feat`, `fix`, `chore`, `refactor`, `test`, `style`.

**Примеры**:
- `docs: add project constitution and spec-kit artifacts`
- `feat(frontend): add voice recording via Web Speech API`
- `chore: remove copilot-specific artifacts from repo`

### Workflow (PR-based)

Все изменения — через ветки и Pull Request:

1. `git checkout -b feature/xxx` или `fix/xxx`
2. Изменения + коммиты
3. `git push origin feature/xxx`
4. Создать PR в main
5. CI проходит (lint, test, build) → мёрдж
6. После мёрджа в main — деплой (ci.yml deploy job)

**Прямой пуш в main запрещён.** Исключение: hotfix (с пометкой `fix:`).

### Что НЕ коммитить
`.clj-kondo/`, `.lsp/`, `.omo/`, `.github/`, `.vscode/` — в `.gitignore`.
- Никаких API-ключей, паролей, SSH-ключей — только в GitHub Secrets.

### Что коммитить
`.specify/` — проектные артефакты (конституция, spec, plan).
